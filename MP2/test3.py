...
def NonmaximaSuppress(mag_image, theta):
    ...
    # print(result)EdgeLinking(Mag_Img, Mag_weak, Mag_strong)
    return result

    def EdgeLinking(Mag_img, Mag_weak, Mag_strong):
        low = 128
        high = 255

        imgShape = Mag_img.shape
        result = np.zeros(imgShape)

        lowX, lowY = np.where((Mag_img > Mag_weak) & (Mag_img <= Mag_strong))
        highX, highY = np.where(Mag_img >= Mag_strong)
        
        result[lowX, lowY] = low
        result[highX, highY] = high
...
if __name__ == "__main__":
    ...
    # Not Needed For 100%
    E = EdgeLinking(Mag_img, Mag_weak, Mag_strong)
